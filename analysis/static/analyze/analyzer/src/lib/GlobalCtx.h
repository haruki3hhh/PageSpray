...
//typedef std::unordered_map<std::string, CallsiteInfo *> LeakStructMap;
...
static std::set<llvm::StringRef> DirectPageAlloc = {
    "alloc_pg_vec",
    "alloc_page",
    "alloc_pages",
    "__get_free_pages",
...
// Copy Root APIs
static std::set<llvm::StringRef> CopyAPIs = {
    "copy_page_from_iter",
...
// Remapping Root APIs: remap a page from kernel-space to user-space
static std::set<llvm::StringRef> RemapAPIs = {
    "remap_pfn_range",  
    "vm_insert_page",
    "vm_iomap_memory",
    "remap_vmalloc_range",
...
public:
  ...
  // in mmapRefList we saved all the function in: `.mmap = func_mmap` to make sure it will be referenced by the related ops structs
  ...
  CalleeMap Callees;							    // typedef llvm::DenseMap<llvm::CallInst *, FuncSet> CalleeMap;
